...
async function fetchData(http, limit, page) {
  spinner(true);
  const url = !limit ? `${http}product` : `${http}product?page=${page}&limit=6`;
  const response = await fetch(url);
  const products = await response.json().finally(spinner(false));
  console.log("products:", products);
  maxPage = Math.ceil(products.length / 6);
...
window.addEventListener("load", function (e) {
  ...
  maxPage = fetchData(http, 6, page++, maxPage);
