    ...
    data.forEach((val) => {
      ...
      const actionStatus = !status
        ? `<div class="action-deliverable">
            ...
            <button class="btn-deliverable" data-id="${val?._id}">Processing in Delivery</button>
          </div>`
        : status == 1
        ? `<div class="action-deliverable">
            <button data-id="${val?._id}" class="btn-cancel">Cancel</button>
            <button data-id="${val?._id}" class="btn-delivered">Order Successfully</button>
          </div>`
        : `<div class="action-deliverable">
            <button data-id="${val?._id}" class="btn-delete">Delete</button>
...
export default function adminOrder() {
  ...
  renderOrderElement.addEventListener("click", async function (e) {
    ...
    const btnDelivered = e.target.closest(".btn-delivered");
    if (btnDelivered) {
      const deliveredOrderId = btnDelivered.dataset.id;
      await changeStatus(deliveredOrderId, 2);
      await fetchOrder(page, orderid, date, status);
    }

    if (btnCancel) {
      const cancelOrderId = btnCancel.dataset.id;
      console.log("cancelOrderId:", cancelOrderId);
      await changeStatus(cancelOrderId, 0);
      await fetchOrder(page, orderid, date, status);
    }
    if (btnDeliverable) {
