...
function showModelEdit(dataproduct) {
  ...
  getDataProduct = { name, img, imgBg, imgBg, state, rating } = dataproduct;
...
window.addEventListener("load", function (e) {
  ...
  productItems.addEventListener("click", async function (e) {
    const btnEditId = e.target.closest(".edit").dataset.id;

    if (btnEditId) {
      const getEditData = await fetch(`${http}product/${btnEditId}`);
  ...
  editData.addEventListener("click", function (e) {
    if (getDataProduct) console.log("getDataProduct:", getDataProduct);
  });
