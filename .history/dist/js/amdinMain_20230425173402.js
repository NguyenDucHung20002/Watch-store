...
window.addEventListener("load", async function (e) {
  ...
  const checkProductsPage = false;
  const checkDashboardPage = false;
  const checkCustomersPage = false;
  const checkOrdersPage = false;
  const checkPasswordPage = false;
  const checkMessagesPage = false;
  ...
  products.addEventListener("click", async function (e) {
    ...
    if (!checkProductsPage) await adminProduct();
  });
