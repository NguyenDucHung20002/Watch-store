    ...
    data.forEach((val) => {
      ...
      const actionStatus = !status
        ? `<div class="action-deliverable">
            <button class="btn-deliverable" data-id="${val?.orderId}">Deliverable</button>
          </div>`
        : status == 1
        ? `<div class="action-deliverable">
            <button data-id="${val?.orderId}" class="btn-cancel">Cancel</button>
            <button data-id="${val?.orderId}" class="btn-delivered">Delivered</button>
          </div>`
        : "";
      div.innerHTML = `
      ...
      $(".render-orders").insertAdjacentElement("beforeend", div);
...
export default function adminOrder() {
  ...
  btnTabs.forEach((val) =>
    val.addEventListener("click", function (e) {
      ...
      if (status == 1) {
        $(".order-pending").innerHTML = "";
      }
      if (status == 2) {
        $(".order-successfully").innerHTML = "";
      }
      console.log("hehe");
      fetchOrder(++page, orderid, date, status);
    })
