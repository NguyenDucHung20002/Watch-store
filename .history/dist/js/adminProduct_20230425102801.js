                  ...
                  <div class="product-action">
                    <button class="delete" data-id="${val._id}">
                      ...
                      <div class="wrapper-modal">
                        <div class="model-detele id-${val._id} active">
                            <p>Are you sure!</p>
...
async function editProduct(data, isUpdateNow) {
  try {
    spinner(true);
    await fetch(`${http}product/${isUpdateNow}`, {
      ...
      .then((data) => {
        ...
        // const getPage = Array(page - 1).fill(1);
        // getPage.forEach(async (val, index) => {
        //   await fetchData(http, index + 1);
        // });
        data.data.forEach((val) => renderProducts(val));
        pageNow = Math.ceil(data.data?.length / 6);
        page = pageNow + 1;
        resetModal();
...
async function fetchData(http, pages, searching) {
  spinner(true);
  if (page > pageNow) {
    page = pageNow + 1;
  }
