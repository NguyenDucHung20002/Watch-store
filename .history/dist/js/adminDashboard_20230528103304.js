...
async function myAreaChart() {
  const ctx = document.getElementById("myAreaChart");
  try {
    const url = `${http}bestselling?page=${1}&limit=${5}`;
    const response = await fetch(url, {
      headers: {
        "Content-type": "application/json; charset=UTF-8",
        authentication: User?.token,
    ...
    if (products?.data?.length > 0) {
      ...
      const nameProducts = data.map((val) => val.name);
      const solds = data.map((val) => val.sold);
      new Chart(ctx, {
        type: "polarArea",
        data: {
          labels: nameProducts,
