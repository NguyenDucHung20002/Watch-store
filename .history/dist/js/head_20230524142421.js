...
import Password from "./changePassword.js";
...
export default function head() {
  ...
  const wrapperProfile = $(".wrapper-profile");
  const blockProfile = $(".block-profile");
  const btnCloseFrofile = $(".btn-close-profile");
  if (btnCloseFrofile) {
    btnCloseFrofile.addEventListener("click", function (e) {
      blockProfile.style = "opacity: 0; visibility: hidden;";
    });
  }
  if (blockProfile) {
    blockProfile.addEventListener("click", function (e) {
      blockProfile.style = "opacity: 0; visibility: hidden;";
    });
  }
  if (wrapperProfile) {
    wrapperProfile.addEventListener("click", function (e) {
      e.stopPropagation();
    });
  }

  if (activeScroll) activeScroll.style.width = `${heightActive}px`;
  if (User) {
    const showUser = $(".user");
    showUser.innerHTML = `<i class="fa-solid fa-user"></i> ${User.data?.username}
    <ul class="dropdown-user">
        <li class="profile-link">Your profile</li>
    ...
    profile.addEventListener("click", function () {
      Password();
      blockProfile.style = "opacity: 1; visibility: visible;";
    });
  }
