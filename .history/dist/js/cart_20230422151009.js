...
async function getCart(userId, token) {
  await fetch(`${http}cart/user/${userId}`, {
    ...
    .then((data) => data.json())
    .then((data) => {
      // fetch(`${http}check/product/${data.data?._id}`)
      //   .then((data) => data.json())
      //   .then((data) => {
      //   });
      dataCart = [...data.data];
...
window.addEventListener("load", function () {
  ...
  btnCheckout.addEventListener("click", function (e) {
    ...
    if (dataCart.length > 0) {
      localStorage.setItem("checkout", JSON.stringify(dataCart));
  ...
  addCart.addEventListener("click", function (e) {
    ...
    if (detailImg) {
      const idProduct = detailImg.dataset.id;
      detailImg.setAttribute("href", `./detail.html?idpd=${idProduct}`);
    }
